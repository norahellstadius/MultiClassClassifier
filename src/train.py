...
def datagen(dir_train: str, dir_val: str, dir_test: str, batchSize: int = 32, preproc_func: Callable = tf.keras.applications.resnet.preprocess_input, pixel: int = 224, class_mode: str = "categorical") -> Tuple[DirectoryIterator, DirectoryIterator, DirectoryIterator]:
    """
...
def create_model(train_set: DirectoryIterator, val_set: DirectoryIterator, class_weights: Dict[int, float], num_epochs: int = 5, path_to_save: str = "../models") -> Tuple[Sequential, Any]:
    ...
    num_classes = len(np.unique(train_set.classes))
