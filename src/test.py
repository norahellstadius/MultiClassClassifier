...
def datagen(test_dir: str, batchSize: int =32, pixel: int =224):
    ...
    test_gen = datagenerator.flow_from_directory(test_dir, target_size=(pixel, pixel), class_mode='categorical', batch_size=batchSize, shuffle=False)
    return test_gen
...
def evaluate_model(dir_test: str, model, test_gen: DirectoryIterator, output_dir: str) -> Tuple[float, float, int, int, int, int]:
    ...
    class_labels = list(test_gen.class_indices.keys())  # Get class labels from generator
    ...
    for i in range(len(y_true)):
        if y_pred[i] != y_true[i]:
            ...
            dst = os.path.join(output_dir_misclass, f"true_{class_labels[y_true[i]]}_pred_{class_labels[y_pred[i]]}_{os.path.basename(test_gen.filenames[i])}")
            shutil.copyfile(src, dst)
            print(f"Copied misclassified image: {test_gen.filenames[i]} to {output_dir_misclass}")
            misclassified_counts[class_labels[y_true[i]]] += 1

    # Calculate metrics
    accuracy = accuracy_score(y_true, y_pred)
    auc = roc_auc_score(y_true, y_pred_prob, multi_class='ovr')  # Calculate mean AUC for multiclass
    cm = confusion_matrix(y_true, y_pred, labels=np.arange(len(class_labels)))  # Confusion matrix
...
if __name__ == "__main__":
    ...
    accuracy_pre, auc_pre, misclassified_counts_pre = evaluate_model(args.test_dir, model_pre, test_gen,  "../results/pre")
    print(f'Pre-trained Model - Accuracy: {accuracy_pre:.4f}, AUC Score: {auc_pre:.4f}, Misclassfied Counts: {misclassified_counts_pre}')

    # Evaluate the fully trained model
    accuracy_post, auc_post, misclassified_counts_post = evaluate_model(args.test_dir, model_post, test_gen, "../results/post")
    print(f'Fully Trained Model - Accuracy: {accuracy_post:.4f}, AUC Score: {auc_post:.4f}, Misclassfied Counts: {misclassified_counts_post}')
